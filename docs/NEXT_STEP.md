...
## 下一步唯一目标
- 当前代码收口已达到可交付状态，下一步唯一目标切到“上线 smoke / 观测期”：先用 `./okx config-check --api-check`、`./okx once --dry-run`、`./okx run --dry-run` 在只保留 `BTC-USDT-SWAP`（可选再加 `ETH-USDT-SWAP`）的前提下连续观察配置、通知、TP/SL 说明、LLM 输出与 runtime 退出码是否一致，再决定是否打开真实下单。
...
2. `./okx once --dry-run` / `./okx run --dry-run` 日志中的 `完成/阻断/观望/失败` 统计与实际单标的行为一致。
3. Telegram 只在异常/阻断/下单结果上触发，不再出现悬空通知。
4. 同一套 `protection` 配置在策略说明、执行 attach-algo、回测退出结果上保持一致。
5. 如有参数调整，全量测试仍保持通过，三文件持续同步更新。
