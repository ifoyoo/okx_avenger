...
### 2026-04-23 - D0028 - 默认策略覆盖为激进版
- 背景：最近实盘长时间不交易，根因不是单一阈值，而是默认策略仍被 `1H` 硬门禁、模板硬阻断和实时未收盘 K 线的假流动性一起卡住。
- 决策：
  - 默认策略直接覆盖为激进版，不再保留并行 `aggressive=true` 开关。
  - `1H` 只保留为上下文说明，不再 veto 任何方向。
  - 进入信号、模板和流动性判断统一改用最新确认 K 线；`confirm=0` 的尾 bar 不再参与决策。
  - 模板从门禁改为 booster；无模板但方向明确的 fast-path 允许进场。
  - 默认 watchlist 改为 `BTC/ETH/SOL/XRP/DOGE/SUI` 的混合液态池，默认风控和仓位利用率同步上调。
- 原因：当前默认配置的实际行为太保守，和“希望更激进地交易”这个目标不一致；继续沿用旧默认只会继续制造 HOLD。
- 影响：runtime 现在会明确标记 `entry_tier` 与 `signal_candle_source`，便于区分模板单和 fast-path 单；README / SESSION_STATE / NEXT_STEP 已同步新语义。
- 回滚方案：如后续验证发现过度噪声，可只回滚默认参数和 watchlist，不恢复 `1H` veto 与 open-bar 决策。
