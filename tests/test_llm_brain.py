...
def test_llm_brain_prompt_includes_structured_market_and_intel_context(monkeypatch) -> None:
        ...
        def json(self):
            return {
                "choices": [
                    {
                        "message": {
                            "content": '{"action":"buy","confidence":0.66,"reason":"trend up","risk":"watch resistance"}'
                        }
                    }
                ]
            }
    ...
    monkeypatch.setattr("core.analysis.llm_brain.requests.post", _fake_post)
    features = pd.DataFrame([{"close": 100.0, "rsi": 61.0, "atr": 1.2, "ema_fast": 101.0, "ema_slow": 99.0}])
    ...
    decision = brain.analyze(
        ...
        risk_note=None,
        account_snapshot={"equity": 1000.0, "available": 600.0},
        market_intel={"sentiment_score": 0.4, "event_tags": {"security": 0.8}, "coverage_count": 2},
        ...
        structured_market_intel={
            "sentiment_score": 0.4,
            "event_tags": {"security": 0.8},
    ...
    assert "structured_market_analysis=" in user_prompt
