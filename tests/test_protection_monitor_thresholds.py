...
def test_resolve_inst_threshold_override() -> None:
    monitor = ProtectionMonitor(
        ...
        per_inst_thresholds={
            "BTC-USDT-SWAP": {"take_profit_upl_ratio": 0.03, "stop_loss_upl_ratio": 0.02},
        },
    )
    btc = monitor._resolve_threshold("BTC-USDT-SWAP")
    eth = monitor._resolve_threshold("ETH-USDT-SWAP")

    assert btc.take_profit_upl_ratio == 0.03
    assert btc.stop_loss_upl_ratio == 0.02
    assert eth.take_profit_upl_ratio == 0.1
    assert eth.stop_loss_upl_ratio == 0.05
...
def test_evaluate_position_uses_inst_threshold(monkeypatch) -> None:
    monitor = ProtectionMonitor(
        ...
        thresholds=ProtectionThresholds(take_profit_upl_ratio=0.1, stop_loss_upl_ratio=0.05),
        per_inst_thresholds={
            "BTC-USDT-SWAP": {"take_profit_upl_ratio": 0.04, "stop_loss_upl_ratio": 0.03},
        },
